...
# def create_newonebody(tfbodyrecord_dir,newonebody_dir, cx=256, cy=256):
#     print('Loading newonebody from "%s"' % newonebody_dir)
#     #print('Loading newoneface from "%s"' % newoneface_dir)
#     globbody_pattern = os.path.join(newonebody_dir,  '*.png')
#     #globface_pattern = os.path.join(newoneface_dir,  '*.png')
#     imagebody_filenames = sorted(glob.glob(globbody_pattern))
#     #imageface_filenames = sorted(glob.glob(globface_pattern))
#     print(imagebody_filenames[0])
#     #expected_images = 202599
#     #if len(image_filenames) != expected_images:
#     #    error('Expected to find %d images' % expected_images)
#
#     with TFRecordExporter(tfbodyrecord_dir, len(imagebody_filenames)) as tfr:
#         order = tfr.choose_shuffled_order()
#         for idx in range(order.size):
#             img = np.asarray(PIL.Image.open(imagebody_filenames[idx]))
#             #print(img)
#             assert img.shape == (1500, 1500, 3)
#             img = img[cy - 512 : cy + 512, cx - 512 : cx + 512]
#             #print(img.shape)
#             img = img.transpose(2, 0, 1) # HWC => CHW
#
#             #print(img.shape)
...
def create_newoneface(tffacerecord_dir, newoneface_dir, cx=300, cy=300, ix=128, iy=128):
    print('Loading newoneface from "%s"' % newoneface_dir)
    ...
    with TFRecordExporter(tffacerecord_dir, len(imageface_filenames)) as tfr:
        order1 = tfr.choose_shuffled_order()
        for idx in range(order1.size):
            img = np.asarray(PIL.Image.open(imageface_filenames[idx]))
            assert img.shape == (2 * cx, 2 * cy, 3)
            img = img[cy - iy: cy + iy, cx - ix: cx + ix]
            img = img.transpose(2, 0, 1)  # HWC => CHW
            tfr.add_image(img)
...
def execute_cmdline(argv):
    ...
    p = add_command('create_newonebody', 'Create dataset for NewOnebody.',
                    'create_NewOnebody datasets/NewOnebody ~/downloads/NewOnebody')
    p.add_argument('tfbodyrecord_dir', help='New dataset directory to be created')
    # p.add_argument(     'tffacerecord_dir',     help='New dataset directory to be created')
    p.add_argument('newonebody_dir', help='Directory containing CelebA')
    # p.add_argument(     'newoneface_dir',       help='Directory containing CelebA')
    p.add_argument('--cx', help='Center X coordinate (default: 89)', type=int, default=300)
    p.add_argument('--cy', help='Center Y coordinate (default: 121)', type=int, default=375)

    p = add_command('create_newoneface', 'Create dataset for NewOneface.',
                    'create_NewOneface datasets/NewOneface ~/downloads/NewOneface')
    # p.add_argument('tfbodyrecord_dir', help='New dataset directory to be created')
    p.add_argument('tffacerecord_dir', help='New dataset directory to be created')
    # p.add_argument('newonebody_dir', help='Directory containing CelebA')
    p.add_argument('newoneface_dir', help='Directory containing CelebA')

...
def setup_snapshot_image_grid(G, faceB_set,faceA_set,
    ...
    gw = 1; gh = 1
    if size == '1080p':
        gw = np.clip(1024 // G.output_shape[3], 3, 32)
        gh = np.clip(1024 // G.output_shape[2], 2, 32)
    if size == '4k':
        gw = np.clip(3840 // G.output_shape[3], 7, 32)
        gh = np.clip(2160 // G.output_shape[2], 4, 32)

    reals = np.zeros([gw * gh] + faceB_set.shape, dtype=faceB_set.dtype)
    labels = np.zeros([gw * gh, faceB_set.label_size], dtype=faceB_set.label_dtype)
    features = np.zeros([gw * gh] + faceA_set.shape, dtype=faceA_set.dtype)
    featurelabels = np.zeros([gw * gh, faceA_set.label_size], dtype=faceA_set.label_dtype)
    for idx in range(gw * gh):
        x = idx % gw; y = idx // gw
        while True:
            real, label = faceB_set.get_minibatch_np(1)
            reals[idx] = real[0]
            labels[idx] = label[0]
            break
        while True:
            facereal, facelabel = faceA_set.get_minibatch_np(1)
            features[idx] = facereal[0]
            featurelabels[idx] = facelabel[0]
            break

    latents = features

    return (gw, gh), reals, labels, latents,features,featurelabels
...
class TrainingSchedule:
    def __init__(
        ...
        max_minibatch_per_gpu   = {2},       # Resolution-specific maximum minibatch size per GPU.
        G_lrate_base            = 0.001,    # Learning rate for the generator.
        G_lrate_dict            = {},       # Resolution-specific overrides.
        D_lrate_base            = 0.001,    # Learning rate for the discriminator.
        D_lrate_dict            = {},       # Resolution-specific overrides.
        tick_kimg_base          = 120,      # Default interval of progress snapshots.
        ...
        self.lod = face_set.resolution_log2
        ...
        self.minibatch =self.minibatch//4                #Please Modify minibatch size in here.
...
def train_progressive_gan(
   ...
   # face_set = dataset.load_dataset(data_dir=config.face_dir, verbose=True, **config.faceset)
    feature_set = dataset.load_dataset(data_dir=config.faceA_dir, verbose=True, **config.faceAset)
    training_set = dataset.load_dataset(data_dir=config.faceB_dir, verbose=True, **config.faceBset)
 #   pose_set = dataset.load_dataset(data_dir=config.pose_dir, verbose=True, **config.poseset)

    #cv2.imshow('result', face_set)
    ...
    with tf.device('/gpu:0'):
        if resume_run_id is not None:
            ...
            G = tfutil.Network('G', num_channels=training_set.shape[0], resolution=training_set.shape[1],faceresolution=feature_set.shape[1], label_size=training_set.label_size, **config.G)
            D = tfutil.Network('D', num_channels=training_set.shape[0]*2, resolution=training_set.shape[1],faceresolution=feature_set.shape[1],  label_size=training_set.label_size, **config.D)
            Gs = G.clone('Gs')
    ...
    with tf.name_scope('Inputs'):
        ...
        varforgauss = tf.placeholder(tf.float32, name='varforgauss', shape=[])
        lrate_in        = tf.placeholder(tf.float32, name='lrate_in', shape=[])
        minibatch_in    = tf.placeholder(tf.int32, name='minibatch_in', shape=[])
        minibatch_split = minibatch_in // config.num_gpus
        features, featureslabels = feature_set.get_minibatch_tf()
        reals, labels   = training_set.get_minibatch_tf()

        reals_split     = tf.split(reals, config.num_gpus)
        labels_split    = tf.split(labels, config.num_gpus)
        features_split = tf.split(features, config.num_gpus)
    ...
    misc.save_image_grid(grid_facereals, os.path.join(result_subdir, 'facereals.png'), drange=feature_set.dynamic_range, grid_size=grid_size)
    misc.save_image_grid(grid_fakes[:,0:3], os.path.join(result_subdir, 'fakes%06d.png' % 0), drange=drange_net, grid_size=grid_size)
    ...
    while cur_nimg < total_kimg * 1000:
        ...
        sched = TrainingSchedule(cur_nimg, training_set,feature_set, **config.sched)
        training_set.configure(sched.minibatch, sched.lod)
        feature_set.configure(sched.minibatch, sched.lod)
        ...
        for repeat in range(minibatch_repeats):
            for _ in range(D_repeats):
                tfutil.run([D_train_op, Gs_update_op], {lod_in: sched.lod, lrate_in: sched.D_lrate, minibatch_in: sched.minibatch,varforgauss: svarforgauss})
                cur_nimg += sched.minibatch
            tfutil.run([G_train_op], {lod_in: sched.lod, lrate_in: sched.G_lrate, minibatch_in: sched.minibatch,varforgauss: svarforgauss})
            tfutil.run([G_train_op], {lod_in: sched.lod, lrate_in: sched.G_lrate, minibatch_in: sched.minibatch,
                                      varforgauss: svarforgauss})

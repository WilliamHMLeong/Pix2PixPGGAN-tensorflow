...
def g2upscale2d(x, factor=2):
    assert isinstance(factor, int) and factor >= 1
    if factor == 1: return x
    with tf.variable_scope('g2Upscale2D'):
        s = x.shape
        x = tf.reshape(x, [-1, s[1], s[2], 1, s[3], 1])
        x = tf.tile(x, [1, 1, 1, factor, 1, factor])
        x = tf.reshape(x, [-1, s[1], s[2] * factor, s[3] * factor])
...
def convG(x, num_out_layers, kernel_size, stride, rate, activation_fn=tf.nn.elu, normalizer_fn=slim.instance_norm):
    return slim.conv2d(x, num_out_layers, kernel_size, stride, 'SAME', rate=rate, activation_fn=activation_fn,normalizer_fn=slim.instance_norm)
...
def G_paper(
    ...
    **kwargs):                          # Ignore unrecognized keyword args.
    ...
    if face == False:
        ...
        resolution_log2 = int(np.log2(faceresolution))
    ...
    def torgb(x, res,convGtmp = None): # res = 2..resolution_log2
        lod = resolution_log2 - res
        with tf.variable_scope('ToRGB_lod%d' % lod):
            if convGtmp is None:
                return apply_bias(conv2d(x, fmaps=num_channels, kernel=1, gain=1, use_wscale=use_wscale))
            else:
                x = apply_bias(conv2d(x, fmaps=num_channels, kernel=1, gain=1, use_wscale=use_wscale))
                x = tf.transpose(x,[0,2,3,1])
                return tf.concat(values = [x,convGtmp],axis = -1)
    ...
    if structure == 'linear':
        x = block(combo_in, 2)
        images_out_g1= torgb(x, 2)
        for res in range(3, resolution_log2 + 1):
            lod = resolution_log2 - res
            x = block(x, res)
            img = torgb(x, res)
            images_out_g1 = upscale2d(images_out_g1)
            with tf.variable_scope('Grow_lod%d' % lod):
                images_out_g1 = lerp_clip(img, images_out_g1, lod_in - lod)
    ...
    if structure == 'recursive':
        def grow(x, res, lod):
            ...
            return img()
        images_out_g1 = grow(combo_in, 2, resolution_log2 - 2)

    assert images_out_g1.dtype == tf.as_dtype(dtype)
    images_out_g1 = tf.identity(images_out_g1, name='images_out')

    return images_out_g1
